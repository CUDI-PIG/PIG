...
Donde deberá ingresar las credenciales de su cuenta en PIG proporcionadas por el administrador. Si al momento de correr un comando de kubernetes no abre la página de keycloak, como se ve en la image, entonces puede agregar la bandera `--skip-open-browser` al archivo `k8s-setup.sh` para que le imprima la URI en la terminal a donde se redirecciona la página de keycloak. Quedaría el comando de `kubectl` de la siguiente manera
